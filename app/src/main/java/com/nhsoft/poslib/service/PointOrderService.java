...
public class PointOrderService {
    ...
    private static PointOrderService instance;

    public static PointOrderService getInstance(){
        if (instance==null){
            instance=new PointOrderService();
        }
        return instance;
    }
    ...
    public List<PointOrderDetial> getPointDetails(){
        BookResource bookPosSale = BookResourceService.getInstance().getBookPosSale(LibConfig.activeShiftTable.getSystemBookCode(), LibConfig.S_LOCAL_POINT_GOODS);
