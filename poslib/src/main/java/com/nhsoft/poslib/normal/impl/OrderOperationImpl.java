...
import com.nhsoft.poslib.normal.callback.OrderOperationCallback;
...
public class OrderOperationImpl implements OrderOperationCallback {
    ...
    public boolean collectOrder(PosOrder posOrder) {
        ...
        if (TextUtils.isEmpty(posOrder.getOrderNo())) {
            keyGeneratorBizday = KeyGeneratorBizdayService.getInstance().createPosOrderKG(shiftTable.getSystemBookCode(), shiftTable.getBranchNum(), shiftTable.getShiftTableBizday(),
        ...
        OrderService.getInstance().doPayment(posOrder);
        PosCarryLogService.tryCollectionOrder(posOrder);
        return result;
    }
...
//            nowPosOrder.setPosOrderDetails(localposOrderDetails);
    ...
    public List<PosScaleStyleTypeBean> getEnablePayment(String paymentName) {
        BookResource bookPosSale = BookResourceService.getInstance().getBookPosSale(LibConfig.activeShiftTable.getSystemBookCode(), LibConfig.S_PAY_SCALE_STYLE);
        ...
        if (bookPosSale != null) {
            ...
            if(LibConfig.C_PAYMENT_TYPE_PETCARD_NAME.equals(paymentName)){
                posSaleParam = PayStyleService.getInstance().getSettleCardPayList(posSaleParam);
            }else {
                posSaleParam = PayStyleService.getInstance().getSettlePaySaleList(posSaleParam);
            }
        }
        return posSaleParam;
    }
    ...
    public PosOrder createQuitPosOrder2ByAll(PosOrder posOrder,PosOrder oldPosOrder,KeyGeneratorBizday mCurrentPosOrderKGB ){
        ...
        for (PosOrderDetail posOrderDetailold : posOrderDetails1) {
            ...
            if (posOrderDetail.getItemNum() != 0) {
                ...
                PosItem posItem = PosItemService.getInstance().getPosItemByKey(posOrderDetail.getItemNum());
                if (posOrderDetail.getItemGradeNum() != 0) {
                    PosItemGrade posItemGrade = PosItemService.getInstance().getPosItemGradeByKey(posOrderDetail.getItemGradeNum(),posItem.getItem_num());
...
//            PosOrderCalcUtil.calcPosOrderDetail(posOrderDetail);
        ...
        Payment newPayment;
        List<Payment> payments = OrderService.getInstance().getPaymentList(oldPosOrder.getOrderNo());
