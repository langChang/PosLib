...
public class PosItemTerminalService {
    private static PosItemTerminalService instance;

    public static PosItemTerminalService getInstance() {
        if (instance == null) {
            instance = new PosItemTerminalService();
        }
        return instance;
    }
...
//            }
            if (posItemTerminal.getPosItem()==null){
                posItemList.add(PosItemService.getInstance().getPosItemByKey(posItemTerminal.getItem_num().longValue()));
            }else {
