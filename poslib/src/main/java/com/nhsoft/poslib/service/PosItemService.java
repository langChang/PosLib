...
public class PosItemService {
    ...
    private static PosItemService instance;

    public static PosItemService getInstance() {
        if (instance == null) {
            instance = new PosItemService();
        }
        return instance;
    }
...
//        return pos_itemDao.queryBuilder().where(PosItemDao.Properties.Item_category_code.eq(categoryCode)).list();
    ...
    public static List<PosItem> getAllShoWPosItemAndSonPosItemByCode(String categoryCode, int branchNum) {
        ...
        List<ItemCategory> itemCategoryList=ItemCategoryService.getInstance().getAllSecondSonCategoryList(categoryCode);
    ...
    public static List<PosItem> getAllShoWPosItemByCode(String categoryCode, int branchNum) {
        ...
        List<ManagementTemplateDetail> managementTemplateDetails = ManagementTemplateService.getTemplateDetails(LibConfig.activeBranch.getManagement_template_num());
        List<PosItem> posItems;
        if (branchNum != 0) {
            if(LibConfig.activeBranch != null && LibConfig.activeBranch.getBranch_matrix_price_actived()){
