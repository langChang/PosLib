...
public class PayStyleService {
    private static PayStyleService instance;

    public static PayStyleService getInstance() {
        if (instance == null) {
            instance = new PayStyleService();
        }
        return instance;
    }
...
//        typeBeanList.add(0, posScaleStyleTypeBean);
            ...
            if (mPosScaleStyleTypeBean.getPosOrderPaymentType().equals("1")) {//  支持前台销售
                ...
                if (mPosScaleStyleTypeBean.getPaymentTypeName().equals("积分消费")) {
                    ...
                    if(RetailPosManager.isOpenCrm()){
                        ...
                        VipCrmPointRate bean = VipCrmPointRateService.getInstance().getBean();
                        if (bean == null){
                           continue;
                        }
    ...
    public VipCardTypeBean getVipCardTypeBean(String systemBookCode, String typeName, String type) {
        ...
        //消费卡类型列表
        BookResource bookPosCardType = BookResourceService.getInstance().getBookPosSale(systemBookCode, typeName);
    ...
    public List<VipStrangeGive> getStrangeGiving(String systemBookCode) {
        ...
        BookResource bookPosCardType = BookResourceService.getInstance().getBookPosSale(systemBookCode, "存款金额赠送");
        if (bookPosCardType != null) {
            String s = XmlParser.xml2json(bookPosCardType.getBookResourceParam());
    ...
    public String getPayStyleBankNum(String payby){
        ...
        if (payby.equals("现金")){
            account_bank_num=AccountBankService.getInstance().getBean().getAccount_bank_num()+"";
        }else {
            PayStyleToCashBank beanByName = PayStyleToCashBankService.getInstance().getBeanByName(payby);
            if (beanByName==null){
            }else {
