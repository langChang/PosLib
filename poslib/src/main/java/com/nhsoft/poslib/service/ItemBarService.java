...
public class ItemBarService {
    ...
    private static ItemBarService instance;
    public static ItemBarService getInstance(){
        if (instance==null){
            instance=new ItemBarService();
        }
        return instance;
    }
    ...
    public List<ItemBar> getAllItemBarGoods(){
        ...
        for (ItemBar itemBar : itemBars1){
            if(itemBar.getItem_bar_code() != null && itemBar.getItem_bar_code().startsWith(LibConfig.saleParamsBean.getPrefixOfWeightItem()) && itemBar.getItem_bar_code().length() == 7&& NumberUtil.isNumeric(itemBar.getItem_bar_code())){
                List<PosItem> posItemByItemBarCode = PosItemService.getInstance().getPosItemByItemBarCode(LibConfig.activeShiftTable.getBranchNum(), itemBar.getItem_bar_code());
