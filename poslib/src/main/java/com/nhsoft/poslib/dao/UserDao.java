...
import com.nhsoft.poslib.model.EffectData;
...
public class UserDao {
    ...
    //派单数据
    public static void saveEffectData(EffectData effectData) {
        if (effectData == null) return;
        SharedPreferenceUtil.saveValue(USER_SP, EFFECT_DATA, new Gson().toJson(effectData));
    }

    public static EffectData getEffectData() {
        String effect_data = SharedPreferenceUtil.getStringValueByKey(USER_SP, EFFECT_DATA);
        if (TextUtils.isEmpty(effect_data)) {
            return null;
        }
        return new Gson().fromJson(effect_data, EffectData.class);
    }
